...
export function TopRootCauses() {
  ...
  const topCauses = rootCauses.slice(0, 4);

  const handleClick = (rootCause: typeof rootCauses[0]) => {
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-foreground">Top Root Causes</h3>
            <p className="text-sm text-muted-foreground">AI-detected patterns</p>
          </div>
        </div>
        <button 
      ...
      <div className="space-y-3">
        {topCauses.map((cause, index) => {
          const styles = impactColors[cause.impact];
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                <AlertTriangle className={cn("w-5 h-5 mt-0.5", styles.text)} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={cn("text-xs font-medium px-2 py-0.5 rounded-full", styles.bg, styles.text)}>
