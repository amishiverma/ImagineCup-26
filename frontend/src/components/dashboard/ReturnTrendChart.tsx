...
import { mockReturnTrendData } from '@/data/mockData';
...
export function ReturnTrendChart() {
  ...
  const beforeData = mockReturnTrendData.slice(0, 4);
  const afterData = mockReturnTrendData;

  const data = showAfterFix ? afterData : beforeData;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-foreground">Return Rate Trend</h3>
          <p className="text-sm text-muted-foreground">Weekly return volume</p>
        </div>
      ...
      {showAfterFix && (
        ...
        >
          <p className="text-sm text-success font-medium">
            ↓ 39% reduction after implementing image accuracy fix
          </p>
        </motion.div>
      )}
    </motion.div>
